  ...
  <view class="report-wrap pt-16">
    ...
    <uni-card :is-shadow="false" is-full class="mt-15">
      ...
      <view class="upload-section mt-15">
        ...
        <view class="upload-grid mt-15 d-f ai-c fxw-w">
          ...
          <view
            class="uni-uploader__file"
            v-for="(image, index) in imageDetailList"
            :key="index"
          >
            <image
              class="uni-uploader__img"
      ...
      <view class="upload-section mt-15">
        ...
        <view class="upload-grid mt-15 d-f ai-c fxw-w">
          ...
          <view
            class="uni-uploader__file"
            v-for="(image, index) in secondImgList"
            :key="index"
          >
            <image
              class="uni-uploader__img"
    ...
    >
      <view class="reason-popup bg_white">
        <view class="title ta-c"> 请选择损坏原因 </view>
        <uni-data-checkbox
          selectedColor="#99BBA0"
          type="primary"
          class="reason-checkbox"
          multiple
          v-model="broken_reason"
          mode="list"
          :localdata="reasonOption"
          @change="change"
        ></uni-data-checkbox>
        ...
        success: (res) => {
          ...
          if (!authStatus) {
            uni.showModal({
              title: '授权失败',
              content:
                '迪雷沃APP需要从您的相机或相册获取图片，请在设置界面打开相关权限',
              success: (res) => {
                if (res.confirm) {
                  uni.openSetting()
                }
...
.reason-popup {
  ...
  ::v-deep {
    .uni-data-checklist
      .checklist-group
      .checklist-box.is--list.is-list-border {
      border-top: none;
    }
